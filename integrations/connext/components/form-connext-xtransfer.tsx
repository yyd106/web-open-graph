...
export function FormConnextXTransfer({
  ...
  const getChain = (domainId: string) => {
    return isMainnet
      ...
      : testnetChains.find(
          (testnetChain) => testnetChain.domain_id == domainId
        );
  };
  ...
  const { data: originBalance, queryKey: originBalanceQueryKey } = useBalance({
    address,
  ...
  const { data: destinationBalance, queryKey: destinationBalanceQueryKey } =
    useBalance({
      address,
      token: getAsset()?.contracts.find(
  ...
  useWatchChain(originBalanceQueryKey, destinationBalanceQueryKey);
  ...
  const sendTx = () => {
    if (approveRequest?.to && !contractApproved) {
    ...
    if (xcallRequest?.to) {
      sendTransaction?.({ ...xcallRequest, to: xcallRequest.to });
    }
  };
  ...
  const buttonAction = () => {
    ...
    if (!isInOriginChain() && originChainData) {
